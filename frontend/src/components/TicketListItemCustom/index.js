...
const useStyles = makeStyles((theme) => ({
  ...
  const handleReopenTicket = async (id) => {
    ...
    const handleAcepptTicket = async (id) => {
        ...
        const msg = `{{ms}} *{{name}}*, Soy *${user?.name}* ¿En que puedo servirte?`;
        const message = {
            read: 1,
            fromMe: true,
            mediaUrl: "",
          ...
          primary={
            <span className={classes.contactNameWrapper}>
              ...
              >
                {ticket.contact.name}
                {profile === "admin" && (
                  <Tooltip title="Espiar Conversación">
                    <VisibilityIcon
        ...
        <span className={classes.secondaryContentSecond} >
          {ticket.status === "pending" && (
            <ButtonWithSpinner
              ...
              size="small"
              loading={loading}
			  //DISEÑO DE PLW INSERTADO el handleChangeTab
...
//            >
//</span>              {i18n.t("ticketsList.buttons.closed")}
//            </ButtonWithSpinner>
