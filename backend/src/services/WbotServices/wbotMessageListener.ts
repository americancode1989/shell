...
export const getBodyMessage = (msg: proto.IWebMessageInfo): string | null => {
  ...
  try {
    ...
    if (!objKey) {
      logger.warn(`#### No encontró el tipo 152: ${type}
${JSON.stringify(msg)}`);
...
const downloadMedia = async (msg: proto.IWebMessageInfo) => {
  ...
  try {
    ...
    // Maneje el error según sus necesidades
  }
...
const handleOpenAi = async (
  ...
  const promptSystem = `
En las primeras respuestas, utiliza el nombre ${sanitizeName(contact.name || "Amigo(a)")} para identificar al cliente.
En tu respuesta, asegúrate de ser preciso y conciso, recordando que debes usar como máximo ${prompt.maxTokens} tokens para evitar truncar el final del mensaje.
Cuando sea adecuado, menciona el nombre del cliente para hacer el servicio más personalizado y educado.
Debes esperar el tiempo adecuado para responder a cada consulta del cliente.
Cuando la respuesta requiera una transferencia al departamento de atención al cliente, comienza tu respuesta con 'Acción: Transferir al sector de atención'.
...
const verifyQueue = async (
  ...
  if (choosenQueue) {
    ...
    /* Tratamiento para el envío de mensajes cuando la cola está fuera del horario laboral */
...
const handleMessage = async (
  ...
  try {
    ...
    try {
      if (!msg.key.fromMe) {
        /**
         * Tratamiento para la evaluación del operador
         */

        //  // desarrollador Ricardo: insistir en responder a la evaluación
        //  const rate_ = Number(bodyMessage);

        //  if ((ticket?.lastMessage.includes('_Insatisfeito_') || ticket?.lastMessage.includes('Por favor evalúa nuestro servicio.')) &&  (!isFinite(rate_))) {
        ...
        //            }`,
        //            {
        //              text: 'Por favor evalúe nuestro servicio.'
        //            }
        //          );
        //        },
        //        1000,
    ...
    try {
      if (!msg.key.fromMe && scheduleType) {
        /**
         * Tratamiento para enviar mensajes cuando la empresa está fuera del horario laboral
         */
        if (
        ...
        if (scheduleType.value === "queue" && ticket.queueId !== null) {
          ...
          /**
           * Tratamiento para el envío de mensajes cuando la cola está fuera del horario de atención.
           */
          const queue = await Queue.findByPk(ticket.queueId);
