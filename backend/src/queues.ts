...
async function handleVerifyQueue(job) {
  logger.info("Buscando servicios perdidos en las colas");
  try {
  ...
  try {
    ...
    if (count > 0) {
      schedules.map(async schedule => {
        ...
        logger.info(`Disparo programado para: ${schedule.contact.name}`);
      });
    }
  } catch (e: any) {
    Sentry.captureException(e);
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        for (let i = 0; i < contactData.length; i++) {
          ...
          logger.info(`Registro enviado para la cola de envío: Campanha=${campaign.id};Contato=${contacts[i].name};delay=${delay}`);
        }
...
async function handleLoginStatus(job) {
  ...
  for (let item of users) {
    try {
      ...
      logger.info(`Usuario pasado a desconectado: ${item.id}`);
    } catch (e: any) {
      Sentry.captureException(e);
    }
  }
...
async function handleInvoiceCreate() {
  logger.info("Iniciando generación de recibos");
        ...
        if (invoice[0]['mycount'] > 0) {
          ...
          /*           let transporter = nodemailer.createTransport({
                    ...
                    const mailOptions = {
                      ...
                      subject: 'Factura generada por el Sistema', // Subject line
                      html: `Hola ${c.name} este es un correo electrónico sobre su factura!<br>
          <br>
