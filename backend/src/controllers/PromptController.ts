...
export const remove = async (
  ...
  try {
    ...
    if (count > 0) return res.status(200).json({ message: "No se pudo eliminar. ¡Verifica si este aviso se está utilizando en las conexiones de Whatsapp!" });
    ...
    return res.status(500).json({ message: "No se pudo eliminar! ¡Verifica si este aviso se está utilizando!" });
  }
};
