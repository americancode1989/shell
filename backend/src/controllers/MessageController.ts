...
export const send = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    if (!whatsapp) {
      throw new Error("No fue posible llevar a cabo la operación.");
    }

    if (messageData.number === undefined) {
      throw new Error("Se requiere el número.");
    }
    ...
    if (Object.keys(err).length === 0) {
      throw new AppError(
        "No se pudo enviar el mensaje, intente de nuevo en unos momentos."
      );
    } else {
